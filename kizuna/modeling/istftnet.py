...
class Generator(CustomOp):
    def __init__(
        ...
        self.conv_post = parametrizations.weight_norm(
...
class AdainResBlk1d(nn.Module):
    def __init__(
        ...
        if upsample == "none":
            ...
            self.pool = parametrizations.weight_norm(
                nn.ConvTranspose1d(
                    dim_in,
                    dim_in,
                    kernel_size=3,
    ...
    def _build_weights(self, dim_in, dim_out, style_dim):
        ...
        if self.learned_sc:
            self.conv1x1 = parametrizations.weight_norm(
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.F0_conv = parametrizations.weight_norm(
            nn.Conv1d(1, 1, kernel_size=3, stride=2, groups=1, padding=1)
        )
        self.N_conv = parametrizations.weight_norm(
            nn.Conv1d(1, 1, kernel_size=3, stride=2, groups=1, padding=1)
        )
