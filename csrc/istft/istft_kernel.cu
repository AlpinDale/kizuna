...
template <typename Ktraits> __global__ void istft_kernel(ISTFTParams params) {
  ...
  for (int frame_offset = 0; frame_offset < Ktraits::kMaxFramesPerBlock;
       frame_offset++) {
    ...
    for (int i = 0; i < kBinsPerThread; i++) {
      ...
      if (bin <= kNFFT / 2) {
        const int64_t offset =
            batch_idx * params.mag_stride + frame_idx * (kNFFT / 2 + 1) + bin;
...
#pragma unroll
      for (int i = 0; i < kBinsPerThread; i++) {
        ...
        if (bin < kNFFT) {
          const int pair_idx =
              (bin & (butterfly_size - 1)) + ((bin >> stage) << (stage + 1));
          if (pair_idx < kNFFT) {
            const float angle =
                -2.0f * M_PI * (bin & (butterfly_size - 1)) / butterfly_stride;
            const float2 twiddle = make_float2(cosf(angle), sinf(angle));

            float2 pair;
            pair.x =
                __shfl_xor_sync(FULL_MASK, complex_spec[i].x, butterfly_size);
            pair.y =
                __shfl_xor_sync(FULL_MASK, complex_spec[i].y, butterfly_size);

            const float2 temp = complex_spec[i];
            complex_spec[i].x =
                temp.x + (twiddle.x * pair.x - twiddle.y * pair.y);
            complex_spec[i].y =
                temp.y + (twiddle.x * pair.y + twiddle.y * pair.x);
          }
        }
      }
      __syncthreads();
    ...
    for (int i = 0; i < kBinsPerThread; i++) {
      ...
      if (bin < params.win_length) {
        ...
        const int64_t out_idx =
            batch_idx * params.out_stride + output_offset + bin;
        if (output_offset + bin < params.out_length &&
            out_idx < params.out_stride * params.batch_size) {
          atomicAdd(&reinterpret_cast<float *>(params.out_ptr)[out_idx],
                    real_val);
        }
      }
    }
          ...
          BOOL_SWITCH(n_fft_pow2 <= 512 && n_fft_pow2 > 256, N512, [&] {
            BOOL_SWITCH(n_fft_pow2 <= 1024 && n_fft_pow2 > 512, N1024, [&] {
              BOOL_SWITCH(n_fft_pow2 <= 2048 && n_fft_pow2 > 1024, N2048, [&] {
                BOOL_SWITCH(
                    n_fft_pow2 <= 4096 && n_fft_pow2 > 2048, N4096, [&] {
                      BOOL_SWITCH(
                          n_fft_pow2 <= 8192 && n_fft_pow2 > 4096, N8192, [&] {
                            ...
                            const int n_frame_blocks =
                                (params.n_frames + Ktraits::kMaxFramesPerBlock -
                                 1) /
                                Ktraits::kMaxFramesPerBlock;
                            dim3 grid(params.batch_size, n_frame_blocks);
                            dim3 block(kNThreads);

                            istft_kernel<Ktraits>
                                <<<grid, block, Ktraits::kSmemSize, stream>>>(
                                    params);
                          });
                    });
              });
...
void istft(torch::Tensor &output, torch::Tensor &magnitude,
           torch::Tensor &phase, torch::Tensor &window, int64_t hop_length,
           bool center, bool normalized) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      at::ScalarType::Half, at::ScalarType::BFloat16, magnitude.scalar_type(),
      "istft",
